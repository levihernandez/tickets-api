...
func main() {
	...
	// Secure
	db, err = pgxpool.Connect(context.Background(), "postgres://julian:<password>@<host>:26257/tickets?sslmode=require&sslcert="+homeCertDir+"/certs/client.julian.crt&sslkey="+homeCertDir+"/certs/client.julian.key&sslrootcert="+homeCertDir+"/certs/ca.crt&application_name=pgx-app&pool_max_conns=20&pool_max_conn_lifetime=300s&pool_max_conn_lifetime_jitter=30s")
	if err != nil {
...
func searchUsers(c *gin.Context) {
	...
	rows, err := db.Query(context.Background(), `SELECT * FROM users WHERE id = $1`, uuidUserID)
	if err != nil {
